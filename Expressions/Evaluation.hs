...
binary_eval _ (Plus _) _ = error "the + operator expects two numbers!"
...
binary_eval _ (Minus _) _ = error "the binary - operator expects two numbers!"

binary_eval (Int x p) (Mod _) (Int y _) = Int (rem x y) p
binary_eval _ (Mod _) _ = error "the % operator expects two ints!"

unary_eval :: Token -> Token -> Token
unary_eval (Minus p) (Int x _) = Int (-x) p
unary_eval (Minus p) (Double x _) = Double (-x) p
unary_eval (Minus p) _ = error "the unary - operator expects a number!"